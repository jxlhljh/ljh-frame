...
Ext.ux.ItemSelector = Ext.extend(Ext.form.Field,  {
    ...
    valueChanged: function(store) {
        ...
        this.hiddenField.dom.value = values.join(this.delimiter.split());
    ...
    setValue:function(value){
    	if(value){
    		...
    		this.reset();
	    	var records = this.fromMultiselect.view.store.getRange();
	    	for(var i=0;i<records.length;i++){
	    		var record = records[i];
	    		for(var j=0;j<values.length;j++){
	    			if(values[j] == record.get(this.valueField)){
	    				this.toMultiselect.view.store.add(record);
	    			}
	    		}
	    	}
	    	records = this.toMultiselect.view.store.getRange();
	    	for(var i=0;i<records.length;i++){
	    		var record = records[i];
	    		this.fromMultiselect.view.store.remove(record);
	    	}
